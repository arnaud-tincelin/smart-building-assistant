...
def gateway(monkeypatch: pytest.MonkeyPatch) -> GatewayModelClient:
    client = GatewayModelClient("https://gw.example/openai/v1", "key", "model-router")
...
def test_governed_scenario_reports_gateway_execution(
    ...
    body = client.post("/ask", json={"question": "Why?", "scenario": "governed"}).json()

    assert body["answer"] == "Cooling load peaks midafternoon."
    assert body["citations"] == []
    assert body["execution"]["routing_mode"] == "gateway"
    assert body["execution"]["selected_model"] == "gpt-5.4-mini-2026-03-17"
...
def test_content_safety_block_is_surfaced(
    ...
    resp = client.post("/ask", json={"question": "bad", "scenario": "governed"})
...
def test_gateway_executes_read_only_mcp_tool_and_continues(
    ...
    gateway = GatewayModelClient(
        "https://gw.example/openai/v1", "key", "model-router",
    ...
    assert result.execution.routing_mode == "gateway"
...
def test_mcp_transport_failures_are_controlled(mcp_gateway, monkeypatch, status):
    ...
    response = TestClient(app).post("/ask", json={"question": "List", "scenario": "governed"})
...
def test_configured_gateway_receives_existing_mcp_endpoint(monkeypatch):
    ...
    gateway_client.get_gateway_client.cache_clear()
    try:
        assert gateway_client.get_gateway_client()._mcp_server_url == "https://gw.example/mcp"
    finally:
        gateway_client.get_gateway_client.cache_clear()
