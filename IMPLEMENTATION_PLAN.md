...
- Optional beats **included**: APIM AI Gateway (model-call governance), APIM-hosted
  MCP tools, and Foundry IQ grounding (RAG on sample building/energy docs).
...
- **Observability:** workspace-based Application Insights connected to the Foundry
  ...
  OpenTelemetry spans and AI Gateway OTLP token metrics with content recording disabled.
- **Gateway:** one Azure API Management AI Gateway tier instance (preview), with a
  native Foundry model provider, structured token policy, runtime key, telemetry
...
Frontend Container App (TypeScript SPA)
   └─ HTTP ─▶ Backend Container App (FastAPI)
            ...
            └─ REST /operations/*
APIM AI Gateway tier ─▶ governed model endpoint + OpenAPI-backed MCP ToolServer ─▶ Foundry connection
...
10. **bicep-apim** — native APIM AI Gateway tier + Foundry model registration,
  structured token limit, runtime key, telemetry exporter, and an OpenAPI-backed
  workspace ToolServer published to Foundry.
11. **bicep-containerapp** — **two** Container Apps (backend + frontend) consuming
    ACR images; backend env vars (Foundry IDs) + MI; frontend env var
...
- **AI Gateway ToolServer** — `AIGateway` uses the preview
  `service/workspaces/toolServers` contract rather than classic `service/apis`.
  Foundry stores the shared gateway `Api-Key` header in a `RemoteTool` connection.
